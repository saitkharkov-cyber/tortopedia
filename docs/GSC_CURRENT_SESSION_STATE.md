...
## 9. Правило разграничения проектов

Чтобы не смешивать историю Git двух рабочих контекстов, проект Tortopedia GSC по умолчанию имеет право менять только GSC-документацию.

Разрешённая область изменений для проекта Tortopedia GSC:

```text
docs/GSC_CURRENT_SESSION_STATE.md
docs/GSC_INDEXING_REGISTER.md   # если такой реестр будет создан позже
```

Проект Tortopedia GSC НЕ должен самостоятельно менять без отдельного явного решения:
...
Если диагностика GSC показывает, что нужно менять сайт, правило такое:

```text
GSC-проект фиксирует диагноз и рекомендуемую правку
→ фактическое изменение сайта выполняется в основном проекте Tortopedia
→ build / publish / commit / push выполняются там
```

Исключение возможно только если пользователь прямо просит выполнить конкретную правку сайта из GSC-проекта.

Рекомендуемый префикс commit message для GSC-документации:

```text
GSC: ...
```

Это позволяет визуально отличать диагностические коммиты от контентных и технических изменений основного проекта.
